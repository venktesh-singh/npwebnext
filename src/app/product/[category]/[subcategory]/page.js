...
export default function Page() {   
  ...
  // Extract category, subcategory, and subsubcategory from the path
  const pathSegments = pathname.split('/').filter(Boolean); // remove empty segments  
  const category = pathSegments[1]; // assuming structure: /product/[subcategory]/[subsubcategory]
  ...
  useEffect(() => {
    if (category && subcategory) {
      const fetchProducts = async () => {
        try {
          const response = await fetch(`${BASE_URL}/products/${category}/${subcategory}}`);
          if (!response.ok) {
  ...
  console.log("Chech SubCategory", products)
  return (
    <>
      <TopBar />
      <Header />
          ...
          <div className="container-fluid">
            <div className="breadcrumb">
              <ul>
                ...
                <li>{ subcategory[0].toUpperCase() + subcategory.slice(1).toLowerCase() } <span>/</span></li>
                <li>{ subsubcategory[0].toUpperCase() + subsubcategory.slice(1).toLowerCase() }</li>
              </ul>
            </div>
          </div>
          <section className="top-section sectionbg1">
            <div className="container-fluid">
              <div className="top-head">
                <h2>{ subsubcategory[0].toUpperCase() + subsubcategory.slice(1).toLowerCase() }</h2>
              </div>
